...
def home_page():
    add_item_button = Button(root, text="Add an Item to Menu", command=lambda: load_add_item_page(), height=9, width=200, bg="#a2a2a2")
    add_item_button.config(font=("Times New Roman", 20, "bold"))
    add_item_button.pack(fill="both", expand=True)

    remove_item_button = Button(root, text="Remove an Item from Menu", command=lambda: load_remove_item_page(), height=9, width=200, bg="#a2a2a2")
    remove_item_button.config(font=("Times New Roman", 20, "bold"))
    remove_item_button.pack(fill="both", expand=True)

    new_order_button = Button(root, text="Create New Order", command=lambda: load_new_order_page(), height=9, width=200, bg="#a2a2a2")
